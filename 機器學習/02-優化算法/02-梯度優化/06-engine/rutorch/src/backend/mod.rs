...
impl Storage {
    ...
    pub fn add_broadcast(&self, b: &Self, cols: usize) -> Self {
        match (self, b) {
            ...
            #[cfg(target_os = "macos")]
            (Storage::Metal(a), Storage::Metal(b_)) => {
                Storage::Metal(metal::add_broadcast(a, b_, cols))
            }
            (Storage::Cuda(a), Storage::Cuda(b_)) => {
                Storage::Cuda(cuda::add_broadcast(a, b_, cols))
            }
            _ => panic!("Backend mismatch"),
        }
    }
    ...
    pub fn softmax_bw(&self, grad_out: &Self, target_grad: &mut Self, rows: usize, cols: usize) {
        match (self, grad_out, target_grad) {
            ...
            #[cfg(target_os = "macos")]
            (Storage::Metal(sm), Storage::Metal(go), Storage::Metal(tg)) => {
                metal::softmax_bw(sm, go, tg, rows, cols)
            }
    ...
    pub fn log_softmax_bw(
        ...
        match (self, grad_out, target_grad) {
            ...
            #[cfg(target_os = "macos")]
            (Storage::Metal(lsm), Storage::Metal(go), Storage::Metal(tg)) => {
                metal::log_softmax_bw(lsm, go, tg, rows, cols)
            }
    ...
    pub fn add_broadcast_bw_b(&self, grad_b: &mut Self, rows: usize, cols: usize) {
        match (self, grad_b) {
            ...
            #[cfg(target_os = "macos")]
            (Storage::Metal(go), Storage::Metal(gb)) => {
                metal::add_broadcast_bw_b(go, gb, rows, cols)
            }
