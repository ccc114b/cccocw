...
impl GpuBuffer {
    pub fn new(data: &[f32]) -> Self {
        ...
        let buffer = METAL_CTX.device.new_buffer_with_data(
    ...
    pub fn to_vec(&self) -> Vec<f32> {
        ...
        unsafe {
            std::ptr::copy_nonoverlapping(ptr, vec.as_mut_ptr(), self.length);
        }
        vec
    }
}
...
pub fn relu_bw(a: &GpuBuffer, grad_out: &GpuBuffer) -> GpuBuffer {
    ...
    dispatch_1d(
        "relu_backward",
        &[&a.buffer, &grad_out.buffer, &grad_in.buffer],
        a.length,
    );
    grad_in
}
...
pub fn sum_bw(grad_out: &GpuBuffer, len: usize) -> GpuBuffer {
    ...
    dispatch_1d_with_u32(
        "sum_backward",
        &[&grad_in.buffer, &grad_out.buffer],
        len,
        len as u32,
    );
    grad_in
...
pub fn softmax_bw(
    sm_out: &GpuBuffer,
    grad_out: &GpuBuffer,
    target_grad: &mut GpuBuffer,
    rows: usize,
    cols: usize,
) {
    dispatch_1d_with_u32(
        "softmax_bw",
        &[&sm_out.buffer, &grad_out.buffer, &target_grad.buffer],
        rows,
        cols as u32,
    );
}
...
pub fn log_softmax_bw(
    ...
    dispatch_1d_with_u32(
        "log_softmax_bw",
        &[&log_sm_out.buffer, &grad_out.buffer, &target_grad.buffer],
        rows,
        cols as u32,
    );
}
...
pub fn add_broadcast_bw_b(grad_out: &GpuBuffer, grad_b: &mut GpuBuffer, rows: usize, cols: usize) {
    dispatch_1d_with_2_u32(
        "add_broadcast_bw_b",
        &[&grad_out.buffer, &grad_b.buffer],
        cols,
        rows as u32,
        cols as u32,
    );
