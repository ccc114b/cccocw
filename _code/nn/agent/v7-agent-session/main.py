...
如果需要更多命令就輸出 <shell>。否則，輸出 <end/> 表示結束："""
    ...
    async def _eval_mode(self, user_input: str, target_exec: str = "") -> str:
        session = self.session_manager.get_current()
        if not session:
            return "錯誤：沒有目前的 session"
        ...
        targets = []
        if target_exec:
            exec = session.get_executor(target_exec)
            if exec:
                targets = [exec]
            else:
                return f"錯誤：找不到 Executor：{target_exec}"

        evaluator = session.create_evaluator(*targets)

        context = evaluator.get_context()
        if targets:
            target_info = f"(評估目標: {', '.join(e.name for e in targets)})"
        else:
            target_info = "(未指定評估目標，可於系統測試時指定)"
        full_prompt = (
    ...
    def run(self):
        ...
        while True:
            ...
            if cmd_lower.startswith("/eval "):
                ...
                desc = parts[0]
                target_exec = parts[1] if len(parts) > 1 else ""
                try:
                    result = asyncio.run(self._eval_mode(desc, target_exec))
                    print(f"\n🤖 [EVAL] {result}\n")
                except Exception as e:
                    print(f"\n⚠️  錯誤：{e}\n")
                continue
