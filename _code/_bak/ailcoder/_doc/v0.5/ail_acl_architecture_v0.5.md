...
v0.5 版本是 **ailcoder** 的核心架構，包含：

1. **ailcoder** - 命令列工具，基於 AIL/ACL 框架
2. **.ail 檔案格式** - 任務描述檔案
3. **AICommander** - 自動回答執行者的問題
4. **角色分工** - Commander → Executor → Handler
...
```ail
# todo.ail - SQLite TODO 應用

## 任務
建立一個 SQLite TODO 網頁應用
...
使用方式：
```bash
ailcoder ail/todo.ail -o ./generated
```
...
| 角色 | 職責 | 檔案系統權限 | 狀態 |
|------|------|-------------|------|
| **ailcoder** | CLI 工具，讀取 .ail 檔案 | - | ✅ 完成 |
| **AIExecutor** | 執行任務，產生 ACL 指令 | ❌ 無 | ✅ 完成 |
| **ACLHandler** | 實際檔案操作 | ✅ 有 | ✅ 完成 |
| **AICommander** | 規劃、回答問題 | ❌ 無 | ⚠️ v0.6 |

**為什麼這樣設計**：
1. **安全** - AI 沒有檔案系統直接權限
2. **分離關注點** - AI 負責思考，Handler 負責執行
3. **可替換** - 可以換成 WebSocketHandler、S3Handler 等
