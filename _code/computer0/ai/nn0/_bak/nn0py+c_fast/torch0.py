...
class Adam:
    def __init__(self, params, lr=0.001, betas=(0.9, 0.999), eps=1e-8):
        ...
        self.eps = eps
        self.m = [np.zeros_like(p._t.data if isinstance(p, Parameter) else p.data) for p in self.params]
        self.v = [np.zeros_like(p._t.data if isinstance(p, Parameter) else p.data) for p in self.params]
        self.t = 0
    ...
    def step(self):
        ...
        for i, p in enumerate(self.params):
            ...
            data = np.array(p._t.data)
            p._t._t._data_ptr[0:p._t._t._size] = (data - update).tolist()
...
class SGD:
    def __init__(self, params, lr=0.01, momentum=0.9):
        ...
        self.velocity = [np.zeros_like(p._t.data if isinstance(p, Parameter) else p.data) for p in self.params]
    ...
    def step(self):
        for i, p in enumerate(self.params):
            ...
            data = np.array(p._t.data)
            p._t._t._data_ptr[0:p._t._t._size] = (data - self.velocity[i]).tolist()
