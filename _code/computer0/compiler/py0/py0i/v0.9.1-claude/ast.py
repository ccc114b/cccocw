...
class AST:
    ...
    def __repr__(self):
        args = ', '.join(f'{f}={getattr(self, f, None)!r}' for f in self._fields)
...
def _tokenize(source):
    ...
    NUM_PAT = re.compile(
        r'0[xX][0-9a-fA-F][0-9a-fA-F_]*'           # hex:  0xFF  0x1_A
        r'|0[bB][01][01_]*'                          # bin:  0b10  0b1_0
        r'|0[oO][0-7][0-7_]*'                       # oct:  0o77
        r'|\d[\d_]*\.\d[\d_]*(?:[eE][+-]?\d[\d_]*)?[jJ]?'  # 1.5  1.5e3j
        r'|\d[\d_]*\.(?:[eE][+-]?\d[\d_]*[jJ]?|[jJ]|(?![_\w]))'  # 1.  1.e3
        r'|\.\d[\d_]*(?:[eE][+-]?\d[\d_]*)?[jJ]?'            # .5  .5e3
        r'|\d[\d_]*[eE][+-]?\d[\d_]*[jJ]?'                   # 1e3
        r'|\d[\d_]*[jJ]'                                       # 1j
        r'|\d[\d_]*'                                           # 42  1_000
    )
    ...
    def emit(kind, val, ln=None, co=None):
...
class _Parser:
    def __init__(self, tokens):
        self.tokens     = tokens
        self.pos        = 0
        self.paren_depth = 0   # () [] {} 深度，>0 時 NL 自動略過

    # ── token 操作 ────────────────────────────────────────────────────────────

    def _skip_nl_in_paren(self):
        """括號內部自動略過 NL token（換行在括號中無語義）。"""
        if self.paren_depth > 0:
            while self.pos < len(self.tokens) - 1 and                   self.tokens[self.pos].kind == TK.NL:
                self.pos += 1
    ...
    def parse_block(self):
        """NEWLINE INDENT stmts DEDENT  ─or─  inline stmt after colon."""
        self.skip_newlines()
        # inline block: 冒號後同行有語句（不換行）
        if self.cur().kind not in (TK.INDENT, TK.NEWLINE, TK.NL, TK.EOF):
    ...
    def parse_funcdef_with_decorators(self):
        decorators = []
        while self.is_op('@'):
            self.advance()
    ...
    def parse_alias(self):
        name_parts = [self.eat(TK.NAME).value]
        while self.is_op('.'):
            self.advance()
    ...
    def _left_binop(self, sub, op_map):
        left = sub()
    ...
    def parse_dict_or_set_from_star(self):
        """{ **d, ... } 開頭的 dict。"""
        keys = []; values = []
        while not self.is_op('}'):
            if self.is_op('**'):
    ...
    def parse_comp_generators(self):
        gens = []
    ...
    def parse_stmt(self):  # noqa: F811  (override)
        """語句解析主入口，支援 @ 裝飾器。"""
        self.skip_newlines()
        t = self.cur()
...
def _make_str_node(raw_parts):
    """
    把一個或多個相鄰字串 token 解析成 Constant 或 JoinedStr（f-string）。
    """
    has_fstr = any(p.startswith(('f', 'F', 'rf', 'fr', 'rF', 'Fr', 'RF', 'FR'))
                   for p in raw_parts)
    if has_fstr:
...
def _parse_fstring_spec(spec_src):
...
def parse(source, filename='<unknown>', mode='eval_or_exec'):
    """
